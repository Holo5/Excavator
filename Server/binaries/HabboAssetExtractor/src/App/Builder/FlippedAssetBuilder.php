...
class FlippedAssetBuilder
{
    ...
    public function extractFlippedImages(): void
    {
        ...
        if(!is_dir($this->savePath . '/images') || !is_dir($this->savePath . '/binaries')) {
        ...
        foreach ($assetsToExtract as $destination => $source) {
            ...
            $source = $this->savePath . '/images/' . substr($source, 0, $strPosition - 1) . '_' . $source . '.png';
            $destination = $this->savePath . '/images/' . substr($destination, 0, $strPosition - 1) . '_' . $destination . '.png';
